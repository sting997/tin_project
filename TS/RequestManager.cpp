...
void RequestManager::requestTicket() {
    ...
    if (privilege_status == 3) {
        ...
        std::string ticket_time_validity_line = dbManager.getTicketTimeValidityLine(
                split_auth_data[0],
                split_auth_data[1]
        );
        std::vector<std::string> split_ticket_time_validity_line = getSplitData(ticket_time_validity_line);

        int ticket_time_validity =
                split_ticket_time_validity_line.size() == 3 ? std::stoi(split_ticket_time_validity_line[2]) : 0;
